    ...
    public function run(Faker $faker)
    {
        ...
        foreach($kosts as $kost){
            for($i=0; $i<rand(1,3); $i++)
            {
                ...
                if($kost->id %2 ==1){
                    ...
                    $findRoomType = RoomType::where('kost_id', $kost->id)->where('name', $randomElement)->first();
                    while($findRoomType){
                        $randomElement = $faker->randomElement($nameCollection[1], $count = 1, $allowDuplicates = false);
                        $findRoomType = RoomType::where('kost_id', $kost->id)->where('name', $randomElement)->first();
                    }
                ...
                $roomType->kost_id = $kost->id;
                $roomType->name = $randomElement;
                $roomType->wide = $faker->numberBetween(2, 5);
                $roomType->lenght = $faker->numberBetween(2, 5);
                $roomType->capacity = $faker->numberBetween(1, 4);
                $roomType->save();
            }
        ...
        foreach($room_types as $room_type){
            ...
            for($i=0; $i<rand(1,3); $i++)
            {
                ...
                $priceLists = PriceList::where('room_type_id',$roomType->id)->get();
                $filter = array();
                foreach($priceLists as $priceList){
                    array_push($filter, $priceList->id);
                }
                $filtered = $collection->reject(function ($value, $key) use($filter) {
                    return in_array($value, $filter);
                });

                $priceList = new PriceList;
                $priceList->room_type_id = $roomType->id;
                $priceList->price = $faker->numberBetween(400000, 2000000);
                $priceList->rent_duration_id = $faker->randomElement($filtered);
                $priceList->save();
            }
